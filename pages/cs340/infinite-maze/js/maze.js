...
class Maze {
    ...
    renderMaze() {
        ...
        // render the players
        if (firstRender) {
            this.renderPlayer(window.x, window.y);
        } else {
    ...
    addBlock(rx, ry, geom, initialLoad = false) {
        if (geom.length % BLOCK_W > 0) {
            console.error("WARNING: grid height not a multiple of "+BLOCK_W);
            return false;
        }
        if (geom[0].length % BLOCK_W > 0) {
            console.error("WARNING: grid width not a multiple of "+BLOCK_W);
            return false;
        }
        ...
        for (var by = 0; by < bh; by++) {
            for (var bx = 0; bx < bw; bx++) {
                ...
                if (gkey in this.grids) {
                    console.error("WARNING: grid already exists at "+gkey);
                    continue;
                }
