...
var _data_sources = {
  "college-data": {
    url: "/covid-analysis/2020-10-02/data/uiuc.csv",
    f: function (row) {
      for (var key in row) {
        if (key != "Date") {
          row[key] = +row[key];
...
var _f_load_allSuccess = function () {
  ...
  _dateObj_today = convertDateToObject("2020-10-02");
...
var doRender = function(chart, isInAnimation = false, target = chart.id) {
  ...
  svg.append("line")
  .attr("x1", daysScale(-16))
  .attr("x2", daysScale(-16))
  .attr("y1", 0)
  .attr("y2", height)
  .attr("stroke", "#000")
  .attr("stroke-width", 2);
  ...
  .attr("x", daysScale(-16))
  .attr("y", 0)
  .attr("width", daysScale(-2) - daysScale(-16))
  .attr("height", height)
  .attr("fill", "#ffc");

  svg.append("text")
  .attr("x", daysScale(-16) + ((daysScale(-2) - daysScale(-16)) / 2) )
  .attr("y", 20)
