...
var reducer_sum_with_key = function(result, value, key) {
  ...
  let obj = result[key];
  let date = value["Date"];
  if (!obj[date]) { obj[date] = { cases: 0, tests: 0 } }
...
var charts = {
  ...
  'ugPositivity-UIUC': {
    self: 'ugPositivity-UIUC',
    id: "chart-ugPositivity-UIUC",
    ...
    showDelta: false,
    //avgData: 7,
    dataSelection_y0: { 'ugPositivity_low': 0.0001, ugPositivity_high: 0.0001, 'ugPositivity': 0.0001, 'testPctUIUCofUS': 0.00001, 'active': 0.01, 'cases': 0.01, 'deaths': 0.01, 'recovered': 0.01, 'new-cases': 0.01, 'tests': 0.01, 'testPositivity': 0.01, 'mortalityRate': 0.01},
    yAxisScale: 'fixed',
    xMax: null, yMax: null, data: null,
    trendline: "default",
    xaxis: "right-6wk",

    dataSelection: 'ugPositivity',
    dataRawSelection: "ugPositivity",
    extraData: ["ugPositivity_low", "ugPositivity_high"],
    extraDataScale: ["graph", "graph"],    
    labelOverride: "% Infected",
    startOfSemester: "2020-08-24",
  },  
...
var getHTMLCountryOptionsToSelect = function(allCountries, selectedCountry, addMetaOptions = true) {
  ...
  for (var country of allCountries) {
    ...
    if (country == "———") { el_option.prop('disabled', true); }
...
var do_process_data = function(data, chart, isSubdata = false) {
  ...
  switch (chart.dataSelection) {
    ...
    case 'ugPositivity':
      ratioData = { n: 'Confirmed_UG', d: 'ugPositivity_mid', raw: 'cases' };
      break;

    case 'ugPositivity_low':
      ratioData = { n: 'Confirmed_UG', d: 'ugPositivity_low', raw: 'cases' };
      break;
  
    case 'ugPositivity_high':
      ratioData = { n: 'Confirmed_UG', d: 'ugPositivity_high', raw: 'cases' };
      break;
...
if (!_data_src) { _data_src = "college-data"; }
...
var _data_sources = {
  "college-data": {
    ...
    f: function (row) {
      ...
      row["Confirmed_UG"] = Math.round(row["UIUC_Confirmed"] * 0.95, 0);
      row["ugPositivity_low"] = 28000;
      row["ugPositivity_mid"] = 24500;
      row["ugPositivity_high"] = 21000;
...
var calculateDataOptions = function(value) {
  ...
  if (['testPositivity', 'mortalityRate', 'testPctUIUCofIL', 'ugPositivity',  'ugPositivity_low', 'ugPositivity_high', 'testPctUIUCofUS', 'testPctUIUCofB10', 'confirmedPctUIUCofB10'].indexOf(valuePieces[0]) != -1) {
...
var generateDataLabel_v3 = function(chart, dType, title = false) {
  ...
  if (title) {
    ...
    else if (dType.baseDataType == 'ugPositivity') { dataLabel += "undergraduates infected"; }  
    else if (dType.baseDataType == 'ugPositivity_low') { dataLabel += "undergraduates infected (lower-bound estimate)"; }  
    else if (dType.baseDataType == 'ugPositivity_high') { dataLabel += "undergraduates infected (upper-bound estimate)"; }  
    else {
    ...
    */
    ...
    if (dType.isRatio) {
      ...
      switch (dType.baseDataType) {
        case "ugPositivity":
          n_label = "undergradute cases";
          d_label = "undergradutes on/around campus";
          break;

        case "ugPositivity_low":
          n_label = "undergradute cases";
          d_label = "undergradutes on/around campus (lower-bound estimate)";
          break;

        case "ugPositivity_high":
          n_label = "undergradute cases";
          d_label = "undergradutes on/around campus (upper-bound estimate)";
          break;          
      ...
      if (d_prev) {
        ...
        if (d_n && d_d) {
          s += `<div class="tip-details" style="border-top: solid 1px black; padding-bottom: 2px;"><i>` +
            `${avgString}<b>${ numericFormat(d_n) }</b> ${n_label} / <b>${ numericFormat(d_d) }</b> ${d_label}` +
            `</i></div>`;
        }
      }
  ...
  */
  ...
  var renderLineChartLabels = function (svg, i, data, dasharray) {
    ...
    if (dasharray) {
      ...
      if (textString == "UgPositivity_low") { textString = "Lower-bound"; }
      else if (textString == "UgPositivity_high") { textString = "Upper-bound"; }
    } else {
      if (chart.labelOverride) {
        textString = chart.labelOverride
      } else {
